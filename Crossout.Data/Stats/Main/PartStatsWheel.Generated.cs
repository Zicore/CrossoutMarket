namespace Crossout.Data.Stats.Main
{
    // *** Generator: v0.4 This class is automatically generated, please use the non generated variant of this class ***
    public partial class PartStatsWheel : PartStatsBase
    {
        public PartStatsWheel(string key) : base(key) { }
        public bool ods_export {get;set;}
        public string inherit {get;set;}
        public string @class {get;set;}
        public bool released {get;set;}
        public bool tradeable {get;set;}
        public string item_type {get;set;}
        public string subtype {get;set;}
        public string model {get;set;}
        public string editor_type {get;set;}
        public string ui_part_filter {get;set;}
        public string taglist {get;set;}
        public string ui_part_type_icon {get;set;}
        public string ui_part_type {get;set;}
        public int health {get;set;}
        public bool damageable {get;set;}
        public int rarity {get;set;}
        public string physics_material {get;set;}
        public int physics_mass {get;set;}
        public double water_damping {get;set;}
        public string fusion {get;set;}
        public string fusion_cost {get;set;}
        public int universal_rating {get;set;}
        public int require_faction_level {get;set;}
        public double engine_power_mul {get;set;}
        public int tonnage_add {get;set;}
        public string type {get;set;}
        public int steering_lock {get;set;}
        public string solidSurfaceBehaviour {get;set;}
        public string denseSurfaceBehaviour {get;set;}
        public string crumblySurfaceBehaviour {get;set;}
        public bool important {get;set;}
        public bool salvageable {get;set;}
        public int salvage_Scrap_Common {get;set;}
        public int salvage_Scrap_Rare {get;set;}
        public int salvage_Scrap_Epic {get;set;}
        public string ui_part_series {get;set;}
        public int water_paddle {get;set;}
        public int wheel_height {get;set;}
        public int direct_resist {get;set;}
        public int collision_resist {get;set;}
        public int flame_resist {get;set;}
        public string ui_perks_1_ {get;set;}
        public string require_faction {get;set;}
        public int speed_limit {get;set;}
        public double impulse_resistance_mul {get;set;}
        public int traction {get;set;}
        public double height {get;set;}
        public int blast_resist {get;set;}
        public double damage_transmission {get;set;}
        public int critical_angle {get;set;}
        public int collision_damage {get;set;}
        public string xopedia_keyword {get;set;}
        public int saw_damage {get;set;}
        public int max_damage_speed {get;set;}
    }
}
